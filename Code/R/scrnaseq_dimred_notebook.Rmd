...
results_spca %>%
  mutate(
    `var_exp_by_pc1_vs_pca_%` = var_explained_pc1 / results_prcomp_100genes$sdev[[1]]^2 *100,
    genes_used_pca = sum(abs(results_prcomp_100genes$rotation[,1]) > 1e-6),
    `var_exp_by_pc1_vs_orig_%` = var_explained_pc1 * (ncol(data_zeisel)-1) / total_logcounts_variance * 100
  ) ->
  results_spca

# Show the results as a tibble
results_spca %>%
  select(spca_alpha, `var_exp_by_pc1_vs_pca_%`, genes_used_spca, genes_used_pca,  `var_exp_by_pc1_vs_orig_%` )
...
get_plottable_spca_obj <- function(spca_fit){
  out <- spca_fit
  class(out) <- "princomp"
...
plot_pca_result(
  get_plottable_spca_obj(
    spca_fit_very_sparse
    #results_spca$spca_fit[[5]]
  ), 
  interactive = FALSE,
  dataset = data_zeisel_topgenes, 
  show_celltype = TRUE
)

```

Examining the results of the author's clustering algorithm along with the detected sparse direction that uses only 2 genes (Plp1 and Trf), I can now quite strongly feel that the expression levels of Plp1 and Trf are significantly different in one group of cells (that they named "Oligodendrocytes" based on some expert knowledge from existing literature) than an other (that they named "Pyramidal neurons").

This is a testable hypothesis (and probably an already existing result, but if not, feel free to go and prove it quickly :) ).
...
#   normalize(data_zeisel, return_log=FALSE)[names((gene_count_variances %>% sort(decreasing = TRUE))[1:how_many_genes_to_keep]),] %>% 
#     normcounts() %>%
#     t()
# )
# 
...
# tmp_plot_right <- plot_pca_result(results_prcomp_normalcounts, interactive = FALSE, dataset = data_zeisel_topgenes, show_celltype = TRUE)
# gridExtra::grid.arrange(tmp_plot_left, tmp_plot_right, ncol=2)
```


```{r}
# Just random notes, delete before lecture



# # Reproduce log_counts on data_zeisel_topgenes
# (normalize(data_zeisel)[names((gene_count_variances %>% sort(decreasing = TRUE))[1:how_many_genes_to_keep]),] %>% logcounts())[1:10,1:10]

# # Get normalised counts on data_zeisel_topgenes
# normalize(data_zeisel, return_log=FALSE)[names((gene_count_variances %>% sort(decreasing = TRUE))[1:how_many_genes_to_keep]),] %>% normcounts()
